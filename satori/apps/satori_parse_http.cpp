...
int main() {
  ...
  parser->onHeaderField = [](http_parser const* parser, char const* at, size_t length) {
    std::cout << std::string(at, length) << ": " << std::endl;
    return 0;
  };

  parser->onHeaderValue = [](http_parser const* parser, char const* at, size_t length) {
    std::cout << std::string(at, length) << std::endl;
    return 0;
  };
  ...
  auto const request = std::string("GET /test HTTP/1.1\r\n"
