...
namespace Satori {
  namespace detail {
    ...
    struct Process : Handle<T> {
      ...
      int spawn(std::string const& file, std::vector<std::string> const& arguments, unsigned int flags = 0) {
        // TODO: Not all of the options properties have been ported.
        delete_uv_process_options_t(&options);
        options = {};
        options.exit_cb = [](uv_process_t* h, int64_t exit_status, int term_signal) {
