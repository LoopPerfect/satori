...
public:
   ...
   */
  managed_ptr<FSOpen> openFile(std::string const &path, int flags, int mode,
                               std::function<void(size_t)> const &onOpen) {
  ...
  managed_ptr<FSRead>
  readFile(size_t const fileDescriptor,
           std::function<void(int const, StringView const)> const &onRead,
           std::function<void()> const &onReadEnd) {
  ...
  managed_ptr<FSUnlink> unlink(std::string const &path,
                               std::function<void(int)> const &onUnlink) {
