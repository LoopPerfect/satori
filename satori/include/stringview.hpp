...
struct StringView {
  ...
  template <class T>
  constexpr StringView(T const &buf) : b{buf.base}, e{buf.base + buf.len} {}

  template <size_t n>
  constexpr StringView(char (&cstr)[n]) : b(cstr), e(cstr + n) {}

  StringView(std::string const &str)
      : b(str.c_str()), e(str.c_str() + str.size()) {}

  char const *begin() const { return b; }
  char const *end() const { return e; }
  size_t size() const { return e - b; }

  constexpr operator bool() const { return b < e; }

  std::string toString() const { return {b, e}; }
