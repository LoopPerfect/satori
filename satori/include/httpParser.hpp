...
struct HttpParser : private http_parser {
  ...
  HttpDataCb onUrl = [](http_parser const *, const char *, size_t) {
    return 0;
  };
  ...
  HttpDataCb onBody = [](http_parser const *, const char *, size_t) {
    return 0;
  };
  ...
  size_t execute(StringView sv) { return execute(sv.begin(), sv.size()); }
};
